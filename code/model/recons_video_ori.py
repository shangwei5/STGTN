...
class RECONS_VIDEO(nn.Module):
    ...
    # def forward(self, x, bm, label):   #label: 1 ---> sharp; 0 ----> blur;
    #     if x.ndimension() == 5:
    #         b, n, c, h, w = x.size()
    ...
    #     if bm.ndimension() == 5:
    #         b, n, c, h, w = bm.size()
    #         bm_list = [bm[:, i, :, :, :] for i in range(n)]
    #         bm = torch.cat(bm_list, dim=1)
    #
    #     en_weight = bm.clone()
    #     en_weight[label == 0] = 1 - bm[label == 0]
    #     en_weight[label == 1] = 1
    #     first_scale_inblock = self.inBlock(x * en_weight)
