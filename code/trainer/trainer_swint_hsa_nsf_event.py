...
class Trainer_SWINT_HSA_NSF(Trainer):
    ...
    def make_optimizer(self):
        ...
        # return optim.Adam([{"params": self.model.get_model().event_in.parameters()},
        #                    # {"params": self.model.get_model().event_fusion.parameters()},
        #                    {"params": self.model.get_model().SearchTransfer.parameters(), "lr": 5e-5},
        #                    {"params": self.model.get_model().conv_lv1.parameters(), "lr": 5e-5},
        #                    {"params": self.model.get_model().conv_lv2.parameters(), "lr": 5e-5},
        #                    {"params": self.model.get_model().conv_lv3.parameters(), "lr": 5e-5},
        #                    {"params": self.model.get_model().recons_net.parameters(), "lr": 5e-5},
        #                    {"params": self.model.get_model().swin.parameters(), "lr": 1e-5},
    ...
    def train(self):
        ...
        for batch, (input, gt, event, _) in enumerate(self.loader_train):
            ...
            out = self.model(input, event)  #, bm, label
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for idx_img, (input, gt, event, filename) in enumerate(tqdm_test):
                ...
                if h % self.size_must_mode != 0 or w % self.size_must_mode != 0:
                    in_tensor = F.pad(in_tensor, pad=[0, w-w % self.size_must_mode, 0, h-h % self.size_must_mode, 0, 0], mode='replicate')
                    event_tensor = F.pad(event_tensor, pad=[0, w-w % self.size_must_mode, 0, h-h % self.size_must_mode, 0, 0], mode='replicate')
                out = self.model(input,event)
