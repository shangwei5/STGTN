...
class Inference:
    def __init__(self, args):
        ...
        self.label_path = os.path.join(self.data_path, "label_new_epoch11")
        ...
        with torch.no_grad():
            ...
            for v in videos:
                ...
                for in_seq, gt_seq, pre_seq, sub_seq in zip(input_seqs, gt_seqs, PreSIndex_list, SubSIndex_list):
                    ...
                    print("%s and %s are restoring %s..." % (os.path.basename(new_input_frames[pre_seq[0]]), os.path.basename(new_input_frames[sub_seq[self.n_seq - 1]]), os.path.basename(in_seq[self.n_seq // 2])))
                    ...
                    if h % self.size_must_mode != 0 or w % self.size_must_mode != 0:  # w % 8 !=0 %
                        in_tensor = F.pad(in_tensor,
                                          pad=[0, (self.size_must_mode - w // 2 % self.size_must_mode) * 2, 0,
                                               (self.size_must_mode - h // 2 % self.size_must_mode) * 2, 0, 0],
                                          mode='replicate')
...
if __name__ == '__main__':
    ...
    if args.default_data == 'RealBlur':
        ...
        args.model_path = '/mnt/disk10T/shangwei/code/CDVD-TSP-master/experiment/swint_hsa_nsf/model/model_best.pt'
        args.result_path = '../infer_results/swint_hsa_nsf_real_nonbound_dist7+2_newlabel'
        args.n_colors = 3
        args.n_sequence = 3
        args.patch_size = 200
        args.n_feat = 32
