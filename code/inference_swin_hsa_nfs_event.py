...
import time
import argparse
from model.swint_hsa_nsf_event import STGTN
    ...
    def infer(self):
        with torch.no_grad():
            ...
            for v in videos:
                ...
                event_frames = sorted(glob.glob(os.path.join(self.event_path, v, "*")))
                ...
                for in_seq, gt_seq, event_seq in zip(input_seqs, gt_seqs, event_seqs):   #, bm_seq, label_seq   , bm_seqs, label_seqs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--default_data', type=str, default='CED',
    ...
    if args.default_data == 'CED':
        args.data_path = '/mnt/disk10T/shangwei/data/deblur/CED/test'
        args.model_path = '../experiment/swint_hsa_nsf_event/model/model_best.pt'
